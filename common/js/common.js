...
/**
 * 전화번호 포맷 변경 : onkeyup="phoneFomatter(this);" 형식으로 사용
 *
 */
...
$(function () {
    ...
    $('.numeric').css('imeMode', 'disabled').keypress(function (event) {
        ...
        if (event.which && (event.which != 46 && (event.which < 48 || event.which > 57)) && event.which != 8) {
...
//		html += '<select id="' + sel_id + '" class="selectA sel_file_box">';
        ...
        html += '	<input type="file" name="' + tag_n + '" onchange="fnFileChk(this)"/>';
        html += '</div>';
    }
    ...
    $('#' + sel_id).on({
        'change': function (e) {
            ...
            var txt = '';
            for (var i = 0; i < n; i++) {
                txt += '<input type="file" name="' + tag_n + '" onchange="fnFileChk(this)" /><br/>';
            }
 ...
 * @param fis : 파일
 */
function fnFileChk(fis) {
    var str = fis.value;
    var thumbext = "";
    thumbext = fis.value.substring(str.lastIndexOf(".") + 1).toLowerCase();

    if (thumbext != "jpg" && thumbext != "png" && thumbext != "gif" && thumbext != "jpeg" && thumbext != "pdf" && thumbext != "txt" && thumbext != "hwp" && thumbext != "xlxs" && thumbext != "xls" && thumbext != "zip" && thumbext != "docx" && thumbext != "ppt" && thumbext != "pptx") {
        alert('확장자가 jpg, png, gif, jpeg, pdf, txt, hwp, xlxs, xls, zip ,docx, ppt, pptx 인 파일만 등록 가능합니다.');
        fis.value = null;
        return false;
    } else {
        return true;
...
/**
 * 1개 첨부파일을 obj 안에서는 다시 그릴경우는 폼양식이 갱신이 되며
 * 새로운 곳은 sel_file_box, input_file_wrap 이런식으로 증가하게 되며
 * file name은 upload이며 .csv 파일만 선택 할 수 있다.
 *
 * @param obj : 파일양식 그리영역
 * @param num : 현재 첨부파일 개수
 * @returns
 */
function fnComboStrOneFile(obj) {
    // select박스 id, 파일박스 id
    var sel_id = 'sel_file_box';
    var file_box_id = 'input_file_wrap';
    var tag_n = 'upload';
    ...
    var html = '';
    html += '<div id="' + file_box_id + '" class="input_file_wrap">';
    html += '	<input type="file" name="' + tag_n + '" id="' + tag_n + '" accept=".csv" style="width:100%;"/>';
    html += '</div>';
