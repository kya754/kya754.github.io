...
<head>
    ...
    <script type="text/javascript">
		...
		*/
        ...
        // 사업 담당자 등록폼 이동
        function fnDetail(trnsmit_server_no, data_no, organ_code) {
            $("[name=trnsmit_server_no]").val(trnsmit_server_no);
            $("[name=data_no]").val(data_no);
            $("#aform").attr({action: "/iot/charger/saveFormBsnsCharger.do", method: 'post'}).submit();
        }
                                    ...
                                    <div class="form-group">
                                        <div class="input-group input-group-sm">
                                            <input type="text" class="form-control input-sm" id="sch_srvc_nm2"
                                                   ...
                                                   value="<%=param.getString("sch_svc_nm2")%>"/>
                                ...
                                <table class="table table-hover table-bordered">
                                    ...
                                    <c:forEach var="data" items="${resultList}" varStatus="status">
                                        <tr style="cursor:pointer;cursor:hand;">
                                            ...
                                            <td class="text-center"
                                                onclick="fnDetail('${data.TRNSMIT_SERVER_NO}', '${data.DATA_NO}')">${data.ORGAN_NM}</td>
                                            <td class="text-center"
                                                onclick="fnDetail('${data.TRNSMIT_SERVER_NO}', '${data.DATA_NO}')">${data.SRVC_HRANK_NM}</td>
                                            <td class="text-center"
                                                onclick="fnDetail('${data.TRNSMIT_SERVER_NO}', '${data.DATA_NO}')">${data.SRVC_LRANK_NM}</td>
                                            <td onclick="fnDetail('${data.TRNSMIT_SERVER_NO}', '${data.DATA_NO}')"> ${data.SRVC_NM}</td>
                                            <td class="text-center"
                                                onclick="fnDetail('${data.TRNSMIT_SERVER_NO}', '${data.DATA_NO}')">${data.SERVER_NM}</td>
                                            <td class="text-center"
                                                onclick="fnDetail('${data.TRNSMIT_SERVER_NO}', '${data.DATA_NO}')">${data.USER_NM}
                                                <c:if test="${data.USER_NO_CNT > 1}">
                                                    외 ${data.USER_NO_CNT - 1}명
                                                </c:if>
                                            </td>
                                            <td class="text-center"
                                                onclick="fnDetail('${data.TRNSMIT_SERVER_NO}', '${data.DATA_NO}')">${data.SRVC_STATUS_NM}</td>
                                            <td class="text-center"
                                                onclick="fnDetail('${data.TRNSMIT_SERVER_NO}', '${data.DATA_NO}')">${data.REGIST_YMD}</td>
                                        </tr>
                                    </c:forEach>
